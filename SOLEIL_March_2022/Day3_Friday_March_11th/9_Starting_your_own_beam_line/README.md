...
3. A sample model as it provides a result which goes beyond a simple photon beam. The `PowderN` `Single-crystal` or `Absorption_sample` are good bets. Using even a simple sample makes the model much more convincing scientifically, and it also allows to estimate the photon counts at the detector.
